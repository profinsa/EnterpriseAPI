...
<div id="row_editor">
	...
	foreach($data->editCategories as $key =>$value)
            echo "<li role=\"presentation\"". ( $scope->category == $key ? " class=\"active\"" : "")  ."><a href=\"index.php#/?page=" . $app->page . "&action=" . $scope->action .  "&mode=edit&category=" . urlencode($key) . "&item=" . $scope->item . "\">" . $translation->translateLabel($key) . "</a></li>";
	?>
    </ul>
	...
	<div class="pull-right" style="margin-top:20px">
		...
		?>
	    <?php endif; ?>
	    <a class="btn btn-inverse waves-effect waves-light" href="index.php#/?page=<?php echo $app->page . "&action=" . $scope->action .  ( $scope->mode != "new" ? "&mode=view&category=" . urlencode($scope->category) . "&item=" . $scope->item : "") ; ?>">
    ...
    <script>
	 ...
	 $.post("index.php?page=<?php  echo $app->page .  "&action=" . $scope->action; ?>&update=true", itemData.serialize(), null, 'json')
	  .success(function(data) {
	      console.log('ok');
	      window.location = "index.php#/?page=<?php  echo $app->page .  "&action=" . $scope->action; ?>&mode=view&category=<?php  echo urlencode($scope->category) . "&item=" . $scope->item ; ?>";
	  })
	  .error(function(err){
	      console.log('wrong');
	  });
