...
<html lang="en">
	...
	<div id="wrapper">
	    ...
	    ?>
	    <!--
		 Name of Page: ledgerAccountGroup view

		 Method: renders whole page. 

		 Date created: Nikita Zaharov, 16.02.2016

		 Use: used by controllers/GeneralLedger/ledgerAccountGroup.php for rendering page
		 ...
		 Called from:
		 controllers/GeneralLedger/ledgerAccountGroup.php

		 Calls:
		 translation model
		 ledgerAccountGroup model
		 app as model
		...
		<div class="container-fluid">
			...
			<div class="white-box">
			    <!--
				 ...
				 Contains four pages:
				 + grid
				 main screen of GeneralLedger/ledgerAccountGroup.
				...
				<div id="grid_content" class="row">
					...
					<table id="example23" class="table table-striped">
						...
						foreach($rows as $row){
							...
							echo "<a href=\"index.php?page=GeneralLedger/ledgerAccountGroup&mode=view&category=Main&item=" . $row["GLAccountGroupID"] ."\"><span class=\"grid-action-button glyphicon glyphicon-edit\" aria-hidden=\"true\"></span></a>";
						    if($scope->user["accesspermissions"]["GLDelete"])
							echo "<span onclick=\"deleteItem('" . $row["GLAccountGroupID"] . "')\" class=\"grid-action-button glyphicon glyphicon-remove\" aria-hidden=\"true\"></span>";
						    echo "</td>";
				    ...
				    <div>
					<a class="btn btn-info waves-effect waves-light m-r-10" href="index.php?page=GeneralLedger/ledgerAccountGroup&mode=new&category=Main">
				    ...
				    <script>
					 ...
					 if(confirm("Are you sure?")){
					     ...
					     $.getJSON("index.php?page=GeneralLedger/ledgerAccountGroup&delete=true&GLAccountGroupID=" + item)
					      .success(function(data) {
						  window.location = "index.php?page=GeneralLedger/ledgerAccountGroup";
					      })
					      .error(function(err){
						  console.log('wrong');
					      });
				...
				<div id="row_viewer">
					...
					foreach($data->editCategories as $key =>$value)
					    echo "<li role=\"presentation\"". ( $scope->category == $key ? " class=\"active\"" : "")  ."><a href=\"index.php?page=GeneralLedger/ledgerAccountGroup&mode=view&category=" . $key . "&item=" . $scope->item . "\">" . $translation->translateLabel($key) . "</a></li>";
					?>
				    </ul>
				    <div class="table-responsive">
					<table class="table">
					...
					<!--
					   ...
					   -->
					<a class="btn btn-info waves-effect waves-light m-r-10" href="index.php?page=GeneralLedger/ledgerAccountGroup&mode=edit&category=<?php  echo $scope->category . "&item=" . $scope->item ; ?>">
					    <?php echo $translation->translateLabel("Edit"); ?>
					</a>
					<a class="btn btn-inverse waves-effect waves-light" href="index.php?page=GeneralLedger/ledgerAccountGroup&mode=grid">
				...
				<div id="row_editor">
					...
					//uses $data(charOfAccounts model) as dictionaries which contains list of tab names
					foreach($data->editCategories as $key =>$value)
					    echo "<li role=\"presentation\"". ( $scope->category == $key ? " class=\"active\"" : "")  ."><a href=\"index.php?page=GeneralLedger/ledgerAccountGroup&mode=" . $scope->mode ."&category=" . $key . "&item=" . $scope->item . "\">" . $translation->translateLabel($key) . "</a></li>";
					?>
				    </ul>
				    <form id="itemData" class="form-material form-horizontal m-t-30">
					<input type="hidden" name="GLAccountGroupID" value="<?php echo $scope->item; ?>" />
			    ...
			    ?>
				    ...
				    <script>
				     ...
				     function createItem(){
					 var itemData = $("#itemData");
					 $.post("index.php?page=GeneralLedger/ledgerAccountGroup&new=true", itemData.serialize(), null, 'json')
					  .success(function(data) {
					      console.log('ok');
					      window.location = "index.php?page=GeneralLedger/ledgerAccountGroup";
					  })
				     ...
				     function saveItem(){
					 var itemData = $("#itemData");
					 $.post("index.php?page=GeneralLedger/ledgerAccountGroup&update=true", itemData.serialize(), null, 'json')
					  .success(function(data) {
					      console.log('ok');
					      window.location = "index.php?page=GeneralLedger/ledgerAccountGroup&mode=view&category=<?php  echo $scope->category . "&item=" . $scope->item ; ?>";
					  })
					  .error(function(err){
					      console.log('wrong');
					  });
