<div id="grid_content" class="row">
	...
	<table id="example23" class="table table-striped">
		...
		if(count($rows)){
			...
			if(file_exists(__DIR__ . "/gridRowActions.php"))
			    require __DIR__ . "/gridRowActions.php";
			...
			foreach($row as $key=>$value)
				...
				switch($data->gridFields[$key]["inputType"]){
					...
					switch($key){
                        ...
                        break;
                        case "GLTransactionNumber" :
                        $docs = [];
                        if (key_exists($row["GLTransactionTypeID"], $typeToTable)) {
                            $docs = DB::select("SELECT COUNT(*) from " . $typeToTable[$row["GLTransactionTypeID"]]["table"] . " WHERE CompanyID='" . $user["CompanyID"] . "' AND DivisionID='". $user["DivisionID"] ."' AND DepartmentID='" . $user["DepartmentID"] . "' AND " . $typeToTable[$row["GLTransactionTypeID"]]["field"] . "='" . $row["GLTransactionNumber"] . "'", array());
                        }
            
                        if(count($docs) > 0)
                            echo $drill->getViewLinkByTransactionNumberAndType($value, $row["GLTransactionTypeID"]);
                        else
                            echo $value;
                        break;
	...
	<?php endif; ?>
	<?php
	if(file_exists(__DIR__ . "/gridActions.php"))
	    require __DIR__ . "/gridActions.php";
	?>
    </div>
    <script>
