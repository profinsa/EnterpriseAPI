...
if(key_exists("back", $_GET)){
	...
	$backhref = urldecode(preg_replace("/\.\.\./", "#", $_GET["back"])) . "?back=" . urlencode($_GET["back"]);
    ...
    $backhref = urlencode("$public_prefix/index#/grid/{$ascope["path"]}/grid/Main/all");
    $back = "";
}
...
function renderInput($ascope, $data, $category, $item, $key, $value){
	...
	case "text" :
	    ...
	    echo "<input style=\"display:inline\" type=\"" . (key_exists("hidden", $data->editCategories[$category][$key]) ? "hidden" :  "text") . "\" id=\"". $key ."\" name=\"" .  $key. "\" onchange=\"fillSameInputs('" . $value . "', '" . $key . "', this);\" class=\"form-control\" value=\"";
...
<div id="row_viewer"  style="font-size: 11pt">
	...
	<div class="col-md-6 col-xs-12">
		...
		<span class="col-md-7">
		    <?php renderInput($ascope, $data, "...fields", $headerItem, $whom . "ID", $headerItem[$whom . "ID"]); ?>
		</span>
	    </div>
	</div>
	<div class="col-md-6 col-xs-12">
...
//	 console.log(JSON.stringify(chooserData, null, 4));
     });
    </script>
	...
	<div class="col-md-4 pull-right">
		...
		<div>
			...
			<tbody>
			    <?php
				$item = $ascope["mode"] == 'edit' || $ascope["mode"] == 'view'? $data->getEditItem($ascope["item"], $whom) : $data->getNewItem($ascope["item"], $whom );
	...
	<div class="style-5 col-md-2 pull-right about-order">	
		...
		<?php if (!key_exists("hidden", $data->editCategories["...fields"][$value])):?>
...
//				  });
    ...
    <div class="row" style="margin-top: 20px;">
	    ...
	    <?php
	    $item = $ascope["mode"] == 'edit' || $ascope["mode"] == 'view'? $data->getEditItem($ascope["item"], "Memos") : $data->getNewItem($ascope["item"], "Memos" );
		...
		<?php $currentCompany = $data->getCurrentCompany()[0]; ?>
			<a class="btn btn-info" onclick="<?php echo ($ascope["mode"] == "view" ? "saveItem()" : "createItemAndPrint()"); ?>">
...
<script>
	function saveSubgridItemHook(done){
		createItem(function(request) {
			...
			for(ind in idFields) {
				...
				if (idFields[ind] != "CompanyID" && idFields[ind] != "DivisionID" && idFields[ind] != "DepartmentID") {
					var buf = localStorage.getItem("autorecalcData");
					var recalcData = {};
					var bufKeys = Object.keys(buf.length);
					for (var i = 0; i < bufKeys.length; i++) {
						recalcData[bufKeys[i]] = buf[bufKeys[i]];
					}
					var recalcKey = idFields[ind];
					var recalcValue = insertedData[idFields[ind]];
					recalcData[recalcKey] = recalcValue;
					localStorage.setItem("autorecalcData", JSON.stringify(recalcData));
					$("#subgridData").append('<input type="hidden" name="' + idFields[ind] + '" value="' + insertedData[idFields[ind]] + '">');
				}
			}

			saveSubgridItemHook = null;

			done("<?php echo $public_prefix; ?>/index#/grid/<?php echo $ascope["path"]; ?>/view/Main/" + encodeURIComponent(keyString), keyString);
		});
	}
 ...
 function createItemAndPrint() {
	createItem(function(request){
		...
		var ID = insertedData[idFields[3]];

		var currentCompany = <?php echo json_encode($currentCompany); ?>;

		window.open('<?php echo $public_prefix ."/docreports/" . $data->reportType . "/"?>' + ID);

		if (currentCompany.AccountingCopy == 1) {
			window.open('<?php echo $public_prefix ."/docreports/" . $data->reportType . "accountingcopy/"?>' + ID);
		}

		if (currentCompany.FileCopy == 1) {
			window.open('<?php echo $public_prefix ."/docreports/" . $data->reportType . "filecopy/"?>' + ID);
		}
