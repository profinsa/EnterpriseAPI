<?php
/*
   Name of Page: doc reports serviceorderhistory data source
...
class docReportsData{
    ...
    public function getCurrencySymbol(){
        ...
        $result =  $GLOBALS["capsule"]::select("select I.CurrencyID, C.CurrenycySymbol from OrderHeaderHistory I, CurrencyTypes C WHERE I.CurrencyID=C.CurrencyID and I.OrderNumber='" . $this->id . "' and I.CompanyID='" . $user["CompanyID"] . "' and I.DivisionID='" . $user["DivisionID"] . "' and I.DepartmentID='" . $user["DepartmentID"] . "'", array());
    ...
    public function getHeaderData(){
        ...
        $stmt = $conn->prepare("CALL RptDocServiceOrderHeaderHistorySingle('". $user["CompanyID"] . "','". $user["DivisionID"] ."','" . $user["DepartmentID"] . "', '" . $this->id . "')");
        $rs = $stmt->execute();
    ...
    public function getDetailData(){
        ...
        $conn =  $GLOBALS["capsule"]::connection()->getPdo();
        $stmt = $conn->prepare("CALL RptDocServiceOrderDetailHistorySingle('". $user["CompanyID"] . "','". $user["DivisionID"] ."','" . $user["DepartmentID"] . "', '" . $this->id . "')");
        $rs = $stmt->execute();
        $result = $stmt->fetchAll($conn::FETCH_ASSOC);
