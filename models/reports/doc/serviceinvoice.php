...
/*
  ...
  Date created: Nikita Zaharov, 17.04.2017
...
class docReportsData extends docReportsBase{
    ...
    public function getHeaderData(){
        ...
        $stmt = $conn->prepare("CALL RptDocServiceInvoiceHeaderSingle('". $user["CompanyID"] . "','". $user["DivisionID"] ."','" . $user["DepartmentID"] . "', '" . $this->id . "')");
        $rs = $stmt->execute();
    ...
    public function getDetailData(){
        ...
        $conn =  $GLOBALS["capsule"]::connection()->getPdo();
        $stmt = $conn->prepare("CALL RptDocServiceInvoiceDetailSingle('". $user["CompanyID"] . "','". $user["DivisionID"] ."','" . $user["DepartmentID"] . "', '" . $this->id . "')");
        $rs = $stmt->execute();
        $result = $stmt->fetchAll($conn::FETCH_ASSOC);
