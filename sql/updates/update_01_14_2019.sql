...
-- Stored procedure definition script Receipt_Cash for MySQL
...
-- Stored procedure definition script ReceiptCash_CreateGLTransaction2 for MySQL
-- Generated by (c) Ispirer SQLWays 4.0 Build 219 Licensed to STFB
-- Timestamp: Tue Jul 28 16:20:00 2015
...
-- get the ammount of money from the Invoice
   ...
   IF @SWV_Error <> 0 then
	...
	-- the procedure will return an error code
      SET v_ErrorMessage = 'Currency retrievinge failed';
      ROLLBACK;
...
-- and drop the temporary table
      ...
      SET v_ErrorMessage = 'LedgerTransactions_PostCOA_AllRecords 3 call failed';
      ROLLBACK;
      DROP TEMPORARY TABLE IF EXISTS tt_LedgerDetailTmp;
      IF v_ErrorMessage <> '' then
