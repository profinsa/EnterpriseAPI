CREATE PROCEDURE Bank_PostEarnedInterest (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   SET v_GLOffsetAccount = IFNULL(v_GLOffsetAccount,(SELECT GLBankInterestEarnedAccount
   FROM Companies
   WHERE CompanyID = v_CompanyID AND DivisionID  = v_DivisionID AND  DepartmentID = v_DepartmentID));
   ...
   IF v_ReturnStatus = -1 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostEarnedInterest',v_ErrorMessage,
      v_ErrorID);
   ...
   SET v_GLTransactionDescription = CASE v_GLTransactionReference WHEN N'' THEN CONCAT(N'EINT  ',v_BankDocumentNumber) ELSE v_GLTransactionReference END;
   SET v_BankDocumentNumber = CONCAT(N'EINT ',v_BankDocumentNumber);
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Bank_CreateGLTransaction2(v_CompanyID,v_DivisionID,v_DepartmentID,N'Earned Interest',v_TranDate,
   v_GLTransactionDescription,v_GLTransactionReference,v_BankDocumentNumber,
   v_ConvertedAmount,v_GLCashAccount,v_GLOffsetAccount,1,v_CurrencyID,
   v_CurrencyExchangeRate);

   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then

      SET v_ErrorMessage = 'Unable to post Bank Earned Interest to LedgerTransactions';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostEarnedInterest',v_ErrorMessage,
      v_ErrorID);
      CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'BankTransactionID',
