CREATE PROCEDURE Return_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   SET @SWV_Error = 0;
   CALL Return_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Return_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Return_Delete',v_ErrorMessage,
      v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL Return_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Return_Cancel failed';
      ROLLBACK;
