CREATE PROCEDURE ServiceOrder_CopyAllToHistory (v_CompanyID 	NATIONAL VARCHAR(36),
...
BEGIN
   ...
   IF @SWV_Error <> 0 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'ServiceOrder_CopyAllToHistory',
   ...
   WHILE NO_DATA = 0 DO
      SET @SWV_Error = 0;
      SET v_ReturnStatus = ServiceOrder_CopyToHistory(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber);
      IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
		
		
         CLOSE cOrder;
			
         SET v_ErrorMessage = 'ServiceOrder_CopyToHistory call failed';
         ROLLBACK;
         CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'ServiceOrder_CopyAllToHistory',
         v_ErrorMessage,v_ErrorID);
         CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'','');
