CREATE PROCEDURE Invoice_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   DECLARE v_InvoiceStatus SMALLINT;
   ...
   SET @SWV_Error = 0;
   CALL Invoice_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Invoice_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Invoice_Delete',v_ErrorMessage,
      v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL Invoice_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Invoice_Cancel failed';
      ROLLBACK;
