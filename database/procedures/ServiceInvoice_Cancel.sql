CREATE PROCEDURE ServiceInvoice_Cancel (v_CompanyID NATIONAL VARCHAR(36), 
...
BEGIN
   ...
   select   IFNULL(InvoiceHeader.TransactionTypeID,N''), IFNULL(InvoiceHeader.AmountPaid,0), IFNULL(InvoiceHeader.Total,0), InvoiceHeader.CurrencyID, InvoiceHeader.CurrencyExchangeRate, InvoiceHeader.InvoiceDate, IFNULL(InvoiceHeader.GLSalesAccount,N''), IFNULL(InvoiceHeader.GLSalesAccount,N''), InvoiceHeader.CustomerID, InvoiceDetail.ProjectID INTO v_InvoiceType,v_Amount,v_TotalAmount,v_CurrencyID,v_CurrencyExchangeRate,
   v_InvoiceDate,v_CustomerSalesAcct,v_BankKey,v_CustomerID,v_ProjectID FROM
   InvoiceHeader
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Invoice_UndoCustomerFinancialsUpdateAging(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber);
   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then


      SET v_ErrorMessage = 'Invoice_UndoCustomerFinancialsUpdateAging call failed';
      ROLLBACK;
