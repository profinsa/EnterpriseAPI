CREATE PROCEDURE Purchase_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   SET @SWV_Error = 0;
   CALL Purchase_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_PurchaseNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Purchase_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Purchase_Delete',v_ErrorMessage,
      v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL Purchase_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_PurchaseNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Purchase_Cancel failed';
      ROLLBACK;
