CREATE PROCEDURE Bank_PostCheck (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   IF v_ReturnStatus = -1 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostCheck',v_ErrorMessage,
      v_ErrorID);
   ...
   SET v_GLTransactionDescription = CASE v_GLTransactionReference WHEN N'' THEN CONCAT(N'CHK  ',v_BankDocumentNumber) ELSE v_GLTransactionReference END;
   SET v_BankDocumentNumber = CONCAT(N'CHK ',v_BankDocumentNumber);
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Bank_CreateGLTransaction2(v_CompanyID,v_DivisionID,v_DepartmentID,N'Check',v_TranDate,v_GLTransactionDescription,
   ...
   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
      ...
      SET v_ErrorMessage = 'Unable to post Bank Check to LedgerTransactions';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostCheck',v_ErrorMessage,
      v_ErrorID);
      CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'BankTransactionID',
