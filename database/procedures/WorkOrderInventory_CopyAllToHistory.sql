CREATE PROCEDURE WorkOrderInventory_CopyAllToHistory (v_CompanyID 	NATIONAL VARCHAR(36),
...
BEGIN
   ...
   AND UPPER(WorkOrderType) = 'INVENTORY';
   ...
   IF @SWV_Error <> 0 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'WorkOrderInventory_CopyAllToHistory',
   ...
   WHILE NO_DATA = 0 DO
      SET @SWV_Error = 0;
      SET v_ReturnStatus = WorkOrderInventory_CopyToHistory(v_CompanyID,v_DivisionID,v_DepartmentID,v_WorkOrderNumber);
      IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
		
		
         CLOSE cWorkOrders;
			
         SET v_ErrorMessage = 'WorkOrderInventory_CopyToHistory call failed';
         ROLLBACK;
         CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'WorkOrderInventory_CopyAllToHistory',
         v_ErrorMessage,v_ErrorID);
         CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'','');
