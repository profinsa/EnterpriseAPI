CREATE PROCEDURE ReturnInvoice_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   DECLARE v_ReturnInvoiceStatus SMALLINT;
   ...
   SET @SWV_Error = 0;
   CALL ReturnInvoice_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'ReturnInvoice_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'ReturnInvoice_Delete',v_ErrorMessage,
      v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL ReturnInvoice_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'ReturnInvoice_Cancel failed';
      ROLLBACK;
