CREATE PROCEDURE Bank_PostAdjustment (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   IF v_ReturnStatus = -1 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostAdjustment',v_ErrorMessage,
      v_ErrorID);
   ...
   SET v_GLTransactionDescription = CASE v_GLTransactionReference WHEN N'' THEN CONCAT(N'ADJ  ',v_BankDocumentNumber) ELSE v_GLTransactionReference END;
   SET v_BankDocumentNumber = CONCAT(N'ADJ ',v_BankDocumentNumber);

   SET @SWV_Error = 0;
   IF v_ConvertedAmount > 0 then
      ...
      SET v_ReturnStatus = Bank_CreateGLTransaction2(v_CompanyID,v_DivisionID,v_DepartmentID,N'Adjustment',v_TranDate,v_GLTransactionDescription,
   ...
   ELSE
      ...
      SET v_ConvertedAmount = -v_ConvertedAmount;
	
      SET v_ReturnStatus = Bank_CreateGLTransaction2(v_CompanyID,v_DivisionID,v_DepartmentID,N'Adjustment',v_TranDate,v_GLTransactionDescription,
      v_GLTransactionReference,v_BankDocumentNumber,v_ConvertedAmount,
      v_GLCashAccount,v_GLOffsetAccount,1,v_CurrencyID,v_CurrencyExchangeRate);
   end if;

   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then

      SET v_ErrorMessage = 'Unable to post Bank Adjustment to LedgerTransactions';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostAdjustment',v_ErrorMessage,
      v_ErrorID);
      CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'BankTransactionID',
