CREATE PROCEDURE Order_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   DECLARE v_OrderStatus SMALLINT;
   ...
   SET @SWV_Error = 0;
   CALL Order_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Order_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Order_Delete',v_ErrorMessage,
      v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL Order_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'Order_Cancel failed';
      ROLLBACK;
