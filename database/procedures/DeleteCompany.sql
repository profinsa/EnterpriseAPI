CREATE PROCEDURE DeleteCompany (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   WHILE NO_DATA = 0 DO
      ...
      WHERE syscolumns.id = object_id(CONCAT(N'[dbo].',v_TableName)) AND syscolumns.name = N'CompanyID') then
         ...
         SET v_Sql = CONCAT(v_Sql,'DELETE FROM ',v_TableName,' WHERE CompanyID = @CompanyID;');
         SET v_Sql = CONCAT(v_Sql,'ENABLE TRIGGER ALL ON ',v_TableName,';');
         SET v_ParamDefinition = N'@CompanyID NVARCHAR(36)';
