CREATE PROCEDURE Bank_PostInboundWire (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   IF v_ReturnStatus = -1 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostInboundWire',v_ErrorMessage,
      v_ErrorID);
   ...
   SET v_GLTransactionDescription = CASE v_GLTransactionReference WHEN N'' THEN CONCAT(N'IWR  ',v_BankDocumentNumber) ELSE v_GLTransactionReference END;
   SET v_BankDocumentNumber = CONCAT(N'IWR ',v_BankDocumentNumber);
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Bank_CreateGLTransaction2(v_CompanyID,v_DivisionID,v_DepartmentID,N'Inbound Wire',v_TranDate,v_GLTransactionDescription,
   v_GLTransactionReference,v_BankDocumentNumber,
   v_ConvertedAmount,v_GLOffsetAccount,v_GLCashAccount,1,v_CurrencyID,v_CurrencyExchangeRate);

   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then

      SET v_ErrorMessage = 'Unable to post Bank Inbound Wire to LedgerTransactions';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostInboundWire',v_ErrorMessage,
      v_ErrorID);
      CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'BankTransactionID',
