CREATE PROCEDURE Payroll_Agi_Create_Details (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   WHILE NO_DATA = 0 DO
      ...
      SET @SWV_Error = 0;
      SET v_ReturnStatus = Payroll_Agi_Create_Detail_One2(v_CompanyID,v_DivisionID,v_DepartmentID,v_EmployeeID,v_PayrollID,v_PayrollItemID,
      v_Check);
      IF v_ReturnStatus <> 0 OR @SWV_Error <> 0 then
	
         SET v_ErrorMessage = 'Call enterprise.Payroll_Agi_Create_Detail_One failed';
         ROLLBACK;

         CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Payroll_Agi_Create_Details',v_ErrorMessage,
         v_ErrorID);
