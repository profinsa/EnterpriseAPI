CREATE PROCEDURE RMA_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   SET @SWV_Error = 0;
   CALL RMA_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_PurchaseNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'RMA_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'RMA_Delete',v_ErrorMessage,v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL RMA_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_PurchaseNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'RMA_Cancel failed';
      ROLLBACK;
