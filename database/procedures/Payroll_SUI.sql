CREATE PROCEDURE Payroll_SUI (v_CompanyID NATIONAL VARCHAR(36),
	...
	INOUT v_SUI DECIMAL(19,4) ,INOUT SWP_Ret_Value INT) SWL_return:
BEGIN
   ...
   DECLARE v_SUTARate REAL;
   DECLARE v_SUTAWageBase REAL;
   ...
   select   IFNULL(SUTARate,0), IFNULL(SUTAWageBase,0) INTO v_SUTARate,v_SUTAWageBase FROM	PayrollSetup WHERE
   UPPER(CompanyID) = UPPER(v_CompanyID)
   AND UPPER(DivisionID) = UPPER(v_DivisionID)
   ...
   IF v_YearToDateGross < v_SUTAWageBase then
      IF v_YearToDateGross+v_GROSS <= v_SUTAWageBase then
         SET v_SUI = fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,v_GROSS*(v_SUTARate/100),v_CompanyCurrencyID);
      ELSE
         SET v_SUI = fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,(v_SUTAWageBase -v_YearToDateGross)*(v_SUTARate/100), 
         v_CompanyCurrencyID);
      end if;
   end if;
   ...
   IF @SWV_Error <> 0 then
      ...
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Payroll_SUI',v_ErrorMessage,v_ErrorID);
      SET SWP_Ret_Value = -1;
   end if;
