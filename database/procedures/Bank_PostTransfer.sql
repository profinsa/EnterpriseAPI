CREATE PROCEDURE Bank_PostTransfer (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   select   CurrencyID, CurrencyExchangeRate, IFNULL(TransactionAmount,0), IFNULL(Reference,N''), IFNULL(BankDocumentNumber,N''), GLBankAccount1, GLBankAccount2, IFNULL(Posted,0) INTO v_CurrencyID,v_CurrencyExchangeRate,v_Amount,v_GLTransactionReference,
   ...
   IF v_ReturnStatus = -1 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostTransfer',v_ErrorMessage,
      v_ErrorID);
   ...
   SET v_GLTransactionDescription = CASE v_GLTransactionReference WHEN N'' THEN CONCAT(N'TRFR  ',v_BankDocumentNumber) ELSE v_GLTransactionReference END;
   SET v_BankDocumentNumber = CONCAT(N'TRFR ',v_BankDocumentNumber);
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Bank_CreateGLTransaction2(v_CompanyID,v_DivisionID,v_DepartmentID,N'Transfer',v_TranDate,v_GLTransactionDescription,
   ...
   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
      ...
      SET v_ErrorMessage = 'Unable to post Bank Transfer to LedgerTransactions';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostTransfer',v_ErrorMessage,
      v_ErrorID);
      CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'BankTransactionID',
