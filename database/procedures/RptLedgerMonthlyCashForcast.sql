CREATE PROCEDURE RptLedgerMonthlyCashForcast (v_CompanyID NATIONAL VARCHAR(36),
   ...
   PurchaseNumber,
	...
	TIMESTAMPADD(DAY,(EXTRACT(MONTH FROM PurchaseDueDate) -1)*-1,PurchaseDueDate) AS DueDate
   FROM
   PurchaseHeader
   WHERE
   ...
   OrderNumber,
	...
	fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,Total,v_CompanyCurrencyID) as Total,
	fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,BalanceDue,v_CompanyCurrencyID) as BalanceDue,
	TIMESTAMPADD(DAY,(EXTRACT(MONTH FROM OrderDueDate) -1)*-1,OrderDueDate) AS DueDate
   FROM
   OrderHeader
   WHERE
   CompanyID = v_CompanyID AND
