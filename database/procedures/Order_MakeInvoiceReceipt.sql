CREATE PROCEDURE Order_MakeInvoiceReceipt (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Invoice_CreateFromOrder(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber,v_InvoiceNumber);
   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 OR v_InvoiceNumber = N'' then

      SET v_ErrorMessage = 'Creating invoice failed';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Order_MakeInvoiceReceipt',v_ErrorMessage,
      v_ErrorID);
