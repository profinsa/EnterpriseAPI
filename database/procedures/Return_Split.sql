CREATE PROCEDURE Return_Split (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   SET @SWV_Error = 0;
   SET v_ReturnStatus = Return_Recalc(v_CompanyID,v_DivisionID,v_DepartmentID,v_CancelOrderNumber);
   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
   ...
   AND OrderNumber = v_OrderNumber) > 0 then
      ...
      SET @SWV_Error = 0;
      SET v_ReturnStatus = Return_Recalc(v_CompanyID,v_DivisionID,v_DepartmentID,v_OrderNumber);
      IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
