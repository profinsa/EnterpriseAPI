CREATE PROCEDURE Bank_PostDeposit (v_CompanyID NATIONAL VARCHAR(36),
	v_DivisionID NATIONAL VARCHAR(36),
	v_DepartmentID NATIONAL VARCHAR(36),
	v_BankTransactionID  NATIONAL VARCHAR(36), 
	INOUT v_PostTransfer INT  ,INOUT SWP_Ret_Value INT) SWL_return:
BEGIN
   ...
   SET v_GLOffsetAccount = IFNULL(v_GLOffsetAccount,(SELECT GLARAccount
   FROM Companies
   WHERE CompanyID = v_CompanyID AND DivisionID  = v_DivisionID AND  DepartmentID = v_DepartmentID));
   ...
   CALL LedgerMain_VerifyPeriod2(v_CompanyID,v_DivisionID,v_DepartmentID,v_TranDate,v_PeriodToPost,v_PeriodToClose, v_ReturnStatus);
   IF v_PeriodToClose <> 0 then 
	
	

      COMMIT;
      SET v_PostTransfer = 2;
      SET SWP_Ret_Value = 0;
      LEAVE SWL_return;
   ...
   IF v_ReturnStatus = -1 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostDeposit',v_ErrorMessage,
      v_ErrorID);
   ...
   SET @SWV_Error = 0;
   CALL VerifyCurrency2(v_CompanyID,v_DivisionID,v_DepartmentID,v_TranDate,0,v_CompanyCurrencyID,
   ...
   SET v_GLTransactionDescription = CASE v_GLTransactionReference WHEN N'' THEN CONCAT(N'MDEP  ',v_BankDocumentNumber) ELSE v_GLTransactionReference END;
   SET v_BankDocumentNumber = CONCAT(N'MDEP ',v_BankDocumentNumber);
   ...
   SET @SWV_Error = 0;
   CALL Bank_CreateGLTransaction(v_CompanyID,v_DivisionID,v_DepartmentID,N'Misc Deposit',v_TranDate,v_GLTransactionDescription,
   v_GLTransactionReference,v_BankDocumentNumber,
   v_ConvertedAmount,v_GLCashAccount,v_GLOffsetAccount,1,v_CurrencyID,v_CurrencyExchangeRate, v_ReturnStatus);

   IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then

      SET v_ErrorMessage = 'Unable to post Bank Deposit to LedgerTransactions';
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,DepartmentID,'Bank_PostDeposit',v_ErrorMessage,
      v_ErrorID);
      CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'BankTransactionID',
