CREATE PROCEDURE DebitMemo_CopyAllToHistory (v_CompanyID 	NATIONAL VARCHAR(36),
...
BEGIN
   ...
   IF @SWV_Error <> 0 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'DebitMemo_CopyAllToHistory',v_ErrorMessage,
      v_ErrorID);
   ...
   WHILE NO_DATA = 0 DO
      SET @SWV_Error = 0;
      SET v_ReturnStatus = DebitMemo_CopyToHistory(v_CompanyID,v_DivisionID,v_DepartmentID,v_PurchaseNumber);
      IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
		
		
         CLOSE cPurchaseHeader;
			
         SET v_ErrorMessage = 'DebitMemo_CopyToHistory call failed';
         ROLLBACK;
         CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'DebitMemo_CopyAllToHistory',v_ErrorMessage,
         v_ErrorID);
         CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'','');
