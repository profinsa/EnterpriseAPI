CREATE PROCEDURE Payroll_WithholdingTax_City2 (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   DECLARE v_CityAllowance INT;
   DECLARE v_CityFilingStatus NATIONAL VARCHAR(50);
   ...
   DECLARE v_CityFillingStatus NATIONAL VARCHAR(36);
   ...
   DECLARE PayrollCityTaxTabl CURSOR FOR 
   SELECT	
   OverAmnt,
	NotOver,
	TaxBracket,
   ...
   AND UPPER(StatusType) = UPPER(v_CityFilingStatus)
   ...
   select   IFNULL(CityAllowance,0), CityFilingStatus, IFNULL(Dependents,0), PayFrequency INTO v_CityAllowance,v_CityFilingStatus,v_Dependents,v_PayFrequency FROM PayrollEmployeesDetail WHERE
   CompanyID = v_CompanyID
   AND DivisionID = v_DivisionID
   AND DepartmentID = v_DepartmentID
   ...
   select   IFNULL(StandardDeductSingle,0), IFNULL(StandardDeductJoint,0), IFNULL(Exemption,0), IFNULL(Dependents,0) INTO v_StandardDeductSingle,v_StandardDeductJoint,v_Exemption_money,v_Dependents_money FROM PayrollCityTax WHERE
   CompanyID = v_CompanyID
   AND DivisionID = v_DivisionID
   AND DepartmentID = v_DepartmentID
   AND UPPER(State) = UPPER(v_State)
   ...
   OPEN PayrollCityTaxTabl;
   SET NO_DATA = 0;
   FETCH PayrollCityTaxTabl INTO v_OverAmnt,v_NotOver,v_TaxBracket,v_Cumulative,v_WithholdingStatus;
   SWL_Label:
   WHILE NO_DATA = 0 DO
      ...
      IF v_WithholdingStatus = 'Single' then
         ...
         SET v_NetTaxableIncome = fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,v_AnnualGrossPay -v_StandardDeductSingle -(v_Exemption_money*v_CityAllowance) -(v_Dependents*v_Dependents_money), 
         v_CompanyCurrencyID);
      ELSE
         IF v_WithholdingStatus = 'Married' then
		
            SET v_NetTaxableIncome = fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,v_AnnualGrossPay -v_StandardDeductJoint -(v_Exemption_money*v_CityAllowance) -(v_Dependents*v_Dependents_money), 
            v_CompanyCurrencyID);
         end if;
      end if;
   ...
   PayrollRegister
   SET
   CityTax = v_WithholdingTax,Deductions = v_DEDUCTIONS,NetPay = v_Net
   WHERE
   CompanyID = v_CompanyID
   AND DivisionID = v_DivisionID
   AND DepartmentID = v_DepartmentID
   ...
   IF @SWV_Error <> 0 then
      ...
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'Payroll_WithholdingTax_City',
