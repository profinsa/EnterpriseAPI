CREATE PROCEDURE CreditMemo_Delete (v_CompanyID NATIONAL VARCHAR(36),
...
BEGIN
   ...
   DECLARE v_CreditMemoStatus SMALLINT;
   ...
   SET @SWV_Error = 0;
   CALL CreditMemo_CanDelete2(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber,v_CanDelete);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'CreditMemo_CanDelete failed';
      ROLLBACK;


      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'CreditMemo_Delete',v_ErrorMessage,
      v_ErrorID);
...
SET SWP_Ret_Value = -1;
   ...
   SET @SWV_Error = 0;
   CALL CreditMemo_Cancel2(v_CompanyID,v_DivisionID,v_DepartmentID,v_InvoiceNumber);
   IF @SWV_Error <> 0 then

	
      SET v_ErrorMessage = 'CreditMemo_Cancel failed';
      ROLLBACK;
