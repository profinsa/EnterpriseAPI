CREATE PROCEDURE RMA_CopyAllToHistory (v_CompanyID 	NATIONAL VARCHAR(36),
...
BEGIN
   ...
   AND UPPER(IFNULL(PurchaseHeader.TransactionTypeID,N'')) = 'RMA' 
   AND IFNULL(Received,0) = 1
   AND IFNULL(Paid,0) = 1
   AND UPPER(PurchaseNumber) <> 'DEFAULT';
   ...
   IF @SWV_Error <> 0 then
      ...
      ROLLBACK;
      CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'RMA_CopyAllToHistory',v_ErrorMessage,
      v_ErrorID);
   ...
   WHILE NO_DATA = 0 DO
      SET @SWV_Error = 0;
      SET v_ReturnStatus = RMA_CopyToHistory(v_CompanyID,v_DivisionID,v_DepartmentID,v_PurchaseNumber);
      IF @SWV_Error <> 0 OR v_ReturnStatus = -1 then
		
		
         CLOSE cPurchaseHeader;
			
         SET v_ErrorMessage = 'RMA_CopyToHistory call failed';
         ROLLBACK;
         CALL Error_InsertError(v_CompanyID,v_DivisionID,v_DepartmentID,'RMA_CopyAllToHistory',v_ErrorMessage,
         v_ErrorID);
         CALL Error_InsertErrorDetail(v_CompanyID,v_DivisionID,v_DepartmentID,v_ErrorID,'','');
