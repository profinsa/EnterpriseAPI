CREATE PROCEDURE RptGLLedgerTransaction (v_CompanyID NATIONAL VARCHAR(36),
...
v_LedgerTransactionNumber NATIONAL VARCHAR(36)) BEGIN
   ...
   LedgerTransactionsDetail.GLTransactionAccount as AccountNumber,
	...
	CAST(fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,IFNULL(LedgerTransactionsDetail.GLDebitAmount,0),
   v_CompanyCurrencyID) AS DECIMAL(19,2)) as Debit,
	CAST(fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,IFNULL(LedgerTransactionsDetail.GLCreditAmount,0),
   v_CompanyCurrencyID) AS DECIMAL(19,2)) as Credit,
   ...
   SELECT
   CAST(fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,IFNULL(SUM(IFNULL(LedgerTransactionsDetail.GLDebitAmount,0)),0),
   v_CompanyCurrencyID) AS DECIMAL(19,2)) as TotalDebit,
	CAST(fnRound(v_CompanyID,v_DivisionID,v_DepartmentID,IFNULL(SUM(IFNULL(LedgerTransactionsDetail.GLCreditAmount,0)),0),
   v_CompanyCurrencyID) AS DECIMAL(19,2)) as TotalCredit
   FROM LedgerTransactionsDetail
   INNER Join LedgerTransactions ON
   LedgerTransactions.GLTransactionNumber = LedgerTransactionsDetail.GLTransactionNumber and
   LedgerTransactions.CompanyID = v_CompanyID and
